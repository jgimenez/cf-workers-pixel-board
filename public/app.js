...
async function bootstrap() {
  ...
  els.clean.addEventListener("click", async () => {
    ...
    try {
      ...
      // Clear canvas locally; WS broadcastClear will also update other clients
  ...
  els.recognize.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!res.ok) {
        showToast(`AI error: ${res.status}`);
        return;
      }
      const data = await res.json();
      els.aiResults.innerHTML = `
        <div class="ai-card"><div class="name">Kimi K2.6 <span class="latency">${data.latency.kimi}ms</span></div><div class="answer">${escapeHtml(data.kimi)}</div></div>
        <div class="ai-card"><div class="name">Gemma 4 26B <span class="latency">${data.latency.gemma}ms</span></div><div class="answer">${escapeHtml(data.gemma)}</div></div>
      `;
    } finally {
